...
thrasher_connect(apr_pool_t *pool, webfw2_config_t *config)
{
    ...
    if (apr_sockaddr_info_get(&sockaddr,
		...
		config->thrasher_port, 0, pool) != APR_SUCCESS)
	return NULL;
...
int
thrasher_should_retry(webfw2_config_t *config, 
...
thrasher_recv_boolean(thrasher_pkt_t *pkt, apr_socket_t *sock)
{
    ...
    if (apr_socket_recv(sock, (char *)&resp, 
		&torecv) != APR_SUCCESS)
       return -1;	
	...
	const char *srcaddr, uint32_t ident)
{
	...
	case TYPE_THRESHOLD_v1:
	    pkt = thrasher_create_v1_pkt(rec->pool,
		    (char *)rec->hostname, (char *)rec->uri, 
		    inet_addr(srcaddr),
		    strlen(rec->hostname), 
		    strlen(rec->uri));
	    break;
	case TYPE_THRESHOLD_v2:
	    pkt = thrasher_create_v2_pkt(rec->pool,
		    inet_addr(srcaddr));
	    break;
    ...
    if (apr_socket_send(filter->thrasher_sock, 
		(const char *)pkt->packet, &pkt->len) != APR_SUCCESS)
	return -1;
