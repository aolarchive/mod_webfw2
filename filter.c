...
#else 
    if ((network_search_tbl_from_str(pool, rule->src_addrs, (char *)data))) {
	return(1);
    }
#endif
...
#else
    if ((network_search_tbl_from_str(pool, rule->dst_addrs, (char *)data))) {
	return(0);
    }
#endif
...
{
    network_tree_t **tree;
    network_tree_t  *rtree;
    network_node_t  *pnode;
...
#else
	*tree = network_tbl_init(rule->pool, 512);
#endif
    }
...
#ifdef USE_PATRICIA
    pnode = make_and_lookup(rule->pool, rtree, (char *)network);
#else
    pnode = network_add_node_from_str(rule->pool, rtree, (char *)network);
#endif
